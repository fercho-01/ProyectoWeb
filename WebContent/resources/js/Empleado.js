...
app.controller('lista', function($scope, $cookies, serviceObtenerPeticiones,
		ServiceModificar) {
	...
	$scope.enviar = function(data) {
		...
		if (respuesta != "") {
			ServiceModificar
					.modificar(data, $cookies.get('usuario'), respuesta)
...
app.controller('crear', function($scope, $location, ServiceCrearEmpleado) {
	...
	$scope.crearEmpleado = function() {
		if (pass == pass2) {
			ServiceCrearEmpleado.crear($scope.cedula, $scope.pass,
					$scope.nombre, $scope.email, $scope.rol).success(
					function(data) {
...
app.service('ServiceCrearEmpleado', function($http) {
	this.crear = function(cedulaE, passE, nombreE, emailE, cargoE) {
		console.log(cedulaE + " " + passE + " " + nombreE + " " + emailE + " "
				+ cargoE);
		return $http({
			method : 'PUT',
